...
//   Purchase:   $1 => 10 credits    (supporters buy credits, base rate)
//   Withdrawal: 20 credits => $1     (creators cash out earned credits)
const creditsPerUsdPurchase = 10;
const creditsPerUsdWithdraw = 20;
// Promotional bulk-pricing tiers for credit purchases (per spec). Credit
// amounts NOT listed here fall back to the base rate (10 credits = $1).
const creditPackagePrices = { 100: 10, 300: 25, 800: 60, 1500: 110 };
function usdForCredits(credits) {
  return Object.prototype.hasOwnProperty.call(creditPackagePrices, credits)
    ? creditPackagePrices[credits]
    : credits / creditsPerUsdPurchase;
}
// Per assessment spec: Supporters get 50 credits, Creators get 20 credits
// on first registration. Admins get 0.
const signupBonus = { Supporter: 50, Creator: 20, Admin: 0 };
// Minimum withdrawal is 200 raised credits (= $10 at 20 credits per $1).
...
const verifySupporter = async (req, res, next) => {
  if (req.decoded.role !== 'Supporter') {
    return res.status(403).send({ message: 'Supporter access required' });
  }
  next();
};
...
app.post('/api/campaigns', verifyToken, async (req, res) => {
  try {
    ...
    const { title, category, subCategory, location, shortDescription, story, targetAmount, duration, deadline, minimumContribution, coverImage, team = [], rewards = [] } = req.body;
    const parsedTarget = Number(targetAmount);
    const parsedDuration = Number(duration);
    const parsedMinContribution = Math.max(1, Math.floor(Number(minimumContribution) || 1));
    if (!String(title || '').trim() || !String(category || '').trim() || !Number.isFinite(parsedTarget) || parsedTarget <= 0 || !Number.isInteger(parsedDuration) || parsedDuration < 1 || parsedDuration > 365) {
      return res.status(400).send({ message: 'Provide a title, category, valid funding goal, and a deadline within the next year' });
    }
    const now = new Date();
    // Prefer an explicit deadline date; otherwise derive it from the duration.
    const parsedDeadline = deadline && !Number.isNaN(new Date(deadline).getTime())
      ? new Date(deadline)
      : new Date(now.getTime() + parsedDuration * 86400000);
    const newCampaign = {
      ...
      targetAmount: parsedTarget, duration: parsedDuration, deadline: parsedDeadline, minimumContribution: parsedMinContribution, coverImage: String(coverImage || '').trim(),
      ...
      createdAt: now, status: 'Pending', raised: 0, backers: 0,
    };
  ...
  try {
    ...
    const updateDoc = {
      $set: {
        ...
        deadline: updateData.deadline ? new Date(updateData.deadline) : (campaign.deadline || null),
        minimumContribution: updateData.minimumContribution != null && updateData.minimumContribution !== ''
          ? Math.max(1, Math.floor(Number(updateData.minimumContribution) || 1))
          : (campaign.minimumContribution || 1),
        coverImage: updateData.coverImage || campaign.coverImage,
        team: Array.isArray(updateData.team) ? updateData.team : (campaign.team || []),
        rewards: Array.isArray(updateData.rewards) ? updateData.rewards : (campaign.rewards || []),
...
app.get('/api/dashboard/stats', verifyToken, async (req, res) => {
  try {
    ...
    if (role === 'Admin') {
      const [totalCampaigns, totalUsers, pendingCampaigns, totalWithdrawals, paymentsAgg] = await Promise.all([
        ...
        // Total funding successfully processed across the platform (approved contributions), in credits
        contributionsCollection.aggregate([
          { $match: { status: 'Completed' } },
          { $group: { _id: null, total: { $sum: '$amount' } } },
        ]).toArray(),
      ]);
      const totalPayments = paymentsAgg[0]?.total || 0;
      return res.send({ totalCampaigns, totalUsers, pendingCampaigns, totalWithdrawals, totalPayments });
    }
    ...
    if (role === 'Creator') {
      ...
      const now = new Date();
      // Active campaigns = approved AND deadline not yet passed
      const active = myCampaigns.filter(c => {
      ...
      return res.send({ totalCampaigns: myCampaigns.length, totalRaised, activeCampaigns: active, pendingCampaigns: pending });
    }
    ...
    res.send({ totalContributions: contributions.length, totalContributed, projectsSupported: uniqueCampaigns, pendingContributions, credits: user?.credits || 0 });
  } catch (error) {
...
app.post('/api/credits/checkout-session', verifyToken, async (req, res) => {
  try {
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      line_items: [{ price_data: { currency: 'usd', product_data: { name: `${credits.toLocaleString()} Credits` }, unit_amount: Math.round(usdForCredits(credits) * 100) }, quantity: 1 }],
...
app.post('/api/credits/confirm-checkout', verifyToken, async (req, res) => {
  try {
    ...
    try {
      await creditPurchasesCollection.insertOne({ stripeSessionId: sessionId, userEmail: req.decoded.email, credits, amountUSD: usdForCredits(credits), createdAt: new Date() });
...
app.post('/api/contributions', verifyToken, async (req, res) => {
  try {
    ...
    if (parsedAmount < minContribution) return res.status(400).send({ message: `This campaign requires a minimum contribution of ${minContribution} credits` });
    const deadline = campaign.deadline ? new Date(campaign.deadline) : new Date(new Date(campaign.createdAt).getTime() + campaign.duration * 86400000);
    ...
    // Look up the supporter's display name so creators see a name, not just an email
    const supporterDoc = await usersCollection.findOne({ email: userEmail }, { projection: { name: 1 } });
    // Record contribution
    const newContribution = {
      ...
      supporterName: supporterDoc?.name || userEmail,
      amount: parsedAmount,
...
app.get('/api/contributions/review', verifyToken, async (req, res) => {
  try {
    ...
    // Enrich with supporter names (older contributions may predate the stored supporterName)
    const missingNames = [...new Set(pendingContributions.filter(c => !c.supporterName && c.supporterEmail).map(c => c.supporterEmail))];
    if (missingNames.length) {
      const supporters = await usersCollection.find({ email: { $in: missingNames } }, { projection: { email: 1, name: 1 } }).toArray();
      const nameByEmail = Object.fromEntries(supporters.map(s => [s.email, s.name]));
      pendingContributions.forEach(c => { if (!c.supporterName) c.supporterName = nameByEmail[c.supporterEmail] || c.supporterEmail; });
    }
...
app.post('/api/withdrawals', verifyToken, async (req, res) => {
  try {
    ...
    if (!Number.isInteger(parsedCredits) || parsedCredits < minimumWithdrawalCredits || !['bank', 'paypal', 'stripe', 'bkash', 'rocket', 'nagad'].includes(paymentMethod) || !String(paymentDetails || '').trim()) {
...
app.get('/api/reports', verifyToken, async (req, res) => {
  try {
    ...
    // Enrich with reporter names and campaign titles for older reports that lack them
    const missingReporterEmails = [...new Set(reports.filter(r => !r.reporterName && r.reporterEmail).map(r => r.reporterEmail))];
    if (missingReporterEmails.length) {
      const reporters = await usersCollection.find({ email: { $in: missingReporterEmails } }, { projection: { email: 1, name: 1 } }).toArray();
      const nameByEmail = Object.fromEntries(reporters.map(u => [u.email, u.name]));
      reports.forEach(r => { if (!r.reporterName) r.reporterName = nameByEmail[r.reporterEmail] || r.reporterEmail; });
    }
    const missingTitleIds = [...new Set(reports.filter(r => !r.campaignTitle && r.campaignId).map(r => r.campaignId.toString()))];
    if (missingTitleIds.length) {
      const camps = await campaignsCollection.find({ _id: { $in: missingTitleIds.map(cid => new ObjectId(cid)) } }, { projection: { title: 1 } }).toArray();
      const titleById = Object.fromEntries(camps.map(c => [c._id.toString(), c.title]));
      reports.forEach(r => { if (!r.campaignTitle && r.campaignId) r.campaignTitle = titleById[r.campaignId.toString()] || null; });
    }
    res.send(reports);
  } catch (error) {
...
app.post('/api/reports', verifyToken, async (req, res) => {
  try {
    ...
    const reporterDoc = await usersCollection.findOne({ email: req.decoded.email }, { projection: { name: 1 } });
    const newReport = {
      ...
      reporterName: reporterDoc?.name || req.decoded.email,
      status: 'Pending',
      createdAt: new Date(),
    };
